...
public class ApiService {
	...
	public void setIpadResponseVelocityEngine(
			VelocityEngine ipadResponseVelocityEngine) {
	...
	public void setTxReRequestVelocityEngine(
			VelocityEngine txReRequestVelocityEngine) {
	 ...
	 * @throws SQLException
	 */
	public synchronized String handleRequest(String psentity)
		...
		if (requestXml.isNeedWriteTx()) {
			...
			if (devMode || SocketClient.noticeTCP()) {
				...
				log.warn("read content: "
						+ ReflectionToStringBuilder.toString(responseFile));
				model = rxResponseResolve.resolve(responseFile, model, "操作失败",
						requestXml);
			}
		} else {
			model = rxResponseResolve.resolve(null, model, "操作失败", requestXml);
		}
		response = VelocityEngineUtils.mergeTemplateIntoString(
				ipadResponseVelocityEngine, requestXml.getIpadResponseAction()
						+ ".vm", model);
	...
	public IpadRequestInfo getTxRequest(RequestXml requestXml) {
		...
		if (StringUtils.isNotBlank(sid)) {
			...
			if (requestInfo.getTableId() != null
					&& requestInfo.getTableId().length() > 0) {
	 ...
	 * @return
	 */
	public String merge4TxRequest(IpadRequestInfo requestInfo,
			RequestXml requestXml) {
		Map<String, Object> model = new HashMap<String, Object>();
		model.put("ipad", requestInfo);
		requestXml.setTableId(requestInfo.getTableId());
		model.put("xml", requestXml);
		String txRrequestContent = VelocityEngineUtils.mergeTemplateIntoString(
				txReRequestVelocityEngine, requestXml.getAction() + ".vm",
				model);
